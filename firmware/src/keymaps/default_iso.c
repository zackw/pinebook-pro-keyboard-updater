...
__code __at (0x0815) uint16_t keymap[] = KEYMAP_ISO(
    ...
    R(TAB),   R(Q), R(W), R(E), R(R), R(T), R(Y), FK(4), FK(5), FK(6), FK(11), R(LBRC), R(RBRC), R(ENT),
    R(CAPS),   R(A), R(S), R(D), R(F), R(G), R(H), FK(1), FK(2), FK(3), FK(12), R(QUOT), R(BSLS),
    M(LSFT), R(NUBS), R(Z), R(X), R(C), R(V), R(B), R(N), FK(0), R(COMM), FK(13), FK(14), M(RSFT), FR(6), 
...
ADDR(0x007E) = 0x12; // lenth of packet
